...
* [`src/cljs/trade_tracker_ui/`](src/cljs/trade_tracker_ui/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/trade_tracker_ui/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/trade_tracker_ui/`](test/cljs/trade_tracker_ui/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `trade-tracker-ui` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/trade_tracker_ui/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns trade-tracker-ui.example
  (:require [trade-tracker-ui.config :as config])
